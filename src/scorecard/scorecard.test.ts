...
describe('Scorecard', () => {
    ...
    it('returns null on unclaimed category', () => {
        const points = scorecard.getCategorySum('threes');
        expect(points).toEqual(null);
    });
    it('returns 7 points on claimed category', () => {
        scorecard.claimCategory('three_of_a_kind', [1, 1, 1, 2, 2]);
        const points = scorecard.getCategorySum('three_of_a_kind');
        expect(points).toEqual(7);
    });
    it('returns 0 points on claimed category', () => {
        scorecard.claimCategory('three_of_a_kind', [2, 5, 6, 3, 2, 1]);
        const points = scorecard.getCategorySum('three_of_a_kind');
        expect(points).toEqual(0);
    });
    it('throws an error trying to claim a claimed category', () => {
        scorecard.claimCategory('three_of_a_kind', [4, 4, 4, 3, 2]);
        expect(() => {
            scorecard.claimCategory('three_of_a_kind', [3, 5, 2, 3, 3]);
        }).toThrowError();
    });
    it('returns true when category is claimed', () => {
        scorecard.claimCategory('three_of_a_kind', [3, 3, 3, 2, 1]);
    ...
    it('returns true when category is claimed with 0 points', () => {
        scorecard.claimCategory('three_of_a_kind', [6, 4, 4, 1, 1]);
        const isClaimed = scorecard.isCategoryClaimed('three_of_a_kind');
        expect(isClaimed).toBeTruthy();
    });
